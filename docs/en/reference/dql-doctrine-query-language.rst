...
can be influenced by a so-called "hydration mode" which
specifies a particular way in which a SQL result set is
transformed. Each hydration mode has its own dedicated method on
the ``Query`` class:


-  ``getResult()``: Retrieves a collection of objects. The
...
-  ``getArrayResult()``: Retrieves an array graph (a nested
   array) that is largely interchangeable with the object graph
   generated by ``getResult()`` for read-only purposes.
