...
final class SwitchContextTest extends AbstractTest
{
    ...
    public function testCase(): void
    {
        ...
        $this->switchPracticeContext($practiceA, function () use ($patientEgor, $patentGena): void {
            ...
            self::assertInstanceOf(PrimaryPatInsurance::class, $patientEgor->insurances->first());
            self::assertEquals('Aetna in Practice A', $patientEgor->insurances->first()->insurance->name);
        ...
        $this->switchPracticeContext($practiceB, function () use ($patientEgor, $patentGena): void {
            ...
            self::assertCount(1, $patientEgor->insurances);
            self::assertInstanceOf(PrimaryPatInsurance::class, $patientEgor->insurances->first());
            self::assertEquals('Humana in Practice B', $patientEgor->insurances->first()->insurance->name);

            self::assertCount(2, $patentGena->insurances);
            self::assertInstanceOf(PrimaryPatInsurance::class, $patentGena->insurances->first());
            self::assertInstanceOf(SecondaryPatInsurance::class, $patentGena->insurances->last());
            self::assertEquals('Humana in Practice B', $patentGena->insurances->first()->insurance->name);
            self::assertEquals('Custom in Practice B', $patentGena->insurances->last()->insurance->name);
        });
    }
}
