...
class EntityGeneratorTest extends OrmTestCase
{
    ...
    public function testGeneratedEntityClass()
    {
        ...
        self::assertEquals(new \Doctrine\Common\Collections\ArrayCollection(array($comment)), $book->getComments());
        self::assertInternalType('boolean', $book->removeComment($comment));
        self::assertEquals(new \Doctrine\Common\Collections\ArrayCollection(array()), $book->getComments());
...
',
    ...
    {
        self::assertEquals(1, preg_match('/@var\s+([^\s]+)/',$property->getDocComment(), $matches));
    ...
    {
        self::assertEquals(1, preg_match('/@return\s+([^\s]+)/', $method->getDocComment(), $matches));
        self::assertEquals($type, $matches[1]);
    }
