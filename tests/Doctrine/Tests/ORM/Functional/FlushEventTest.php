...
class FlushEventTest extends \Doctrine\Tests\OrmFunctionalTestCase
{
    ...
    {
        $postFlushListener = new PostFlushListener();
        $this->_em->getEventManager()->addEventListener(Events::postFlush, $postFlushListener);

        $user = new CmsUser;
        $user->username = 'AnotherUser';
        $user->name = 'AnotherName';
        $user->status = 'AnotherStatus';

        $this->_em->persist($user);

        $this->assertFalse($postFlushListener->called);

        $this->_em->flush();

        $this->assertTrue($postFlushListener->called);
    }
}
