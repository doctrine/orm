...
class DDC1452Test extends \Doctrine\Tests\OrmFunctionalTestCase
{
    ...
    public function testIssue()
    {
        ...
        self::assertFalse( $results[0]->entitiesB[0]->entityATo instanceof \Doctrine\ORM\Proxy\Proxy );
        self::assertInstanceOf('Doctrine\Common\Collections\Collection', $results[0]->entitiesB[0]->entityATo->getEntitiesB());
    }
    ...
    public function testFetchJoinOneToOneFromInverse()
    {
        ...
        self::assertFalse($data[0]->user instanceof \Doctrine\ORM\Proxy\Proxy);

        $dql = "SELECT u, a FROM Doctrine\Tests\Models\CMS\CmsUser u INNER JOIN u.address a";
        $data = $this->_em->createQuery($dql)->getResult();

        self::assertFalse($data[0]->address instanceof \Doctrine\ORM\Proxy\Proxy);
    }
}
