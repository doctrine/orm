...
class TypeValueSqlTest extends \Doctrine\Tests\OrmFunctionalTestCase
{
    ...
    public function testTypeValueSqlWithAssociations()
    {
        ...
        $this->assertTrue($entity->id < 0, 'Fetched Entity id negative');
        $this->assertEquals(1, $this->_em->getConnection()->fetchColumn("select id from customtype_parents where id=".abs($entity->id).""), 'Database has stored entity with positive id');
    ...
    public function testSelectDQL()
    {
        ...
        $query = $this->_em->createQuery("select p, p.id, c from \Doctrine\Tests\Models\CustomType\CustomTypeParent p JOIN p.child c where p.id = '" . -abs($parentId) . "'"); // Ensure id is negative
        ...
        $this->assertEquals(-abs($parentId), $result[0]['id']);
