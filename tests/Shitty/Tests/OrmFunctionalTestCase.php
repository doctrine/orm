...
namespace Shitty\Tests;

use Shitty\DBAL\Types\Type;
use Shitty\Tests\EventListener\CacheMetadataListener;
use Shitty\ORM\Cache\Logging\StatisticsCacheLogger;
use Shitty\ORM\Cache\DefaultCacheFactory;
...
abstract class OrmFunctionalTestCase extends OrmTestCase
{
     ...
     *
     * @var \Shitty\Common\Cache\Cache|null
     */
     ...
     *
     * @var \Shitty\DBAL\Connection|null
     */
    ...
    /**
     * @var \Shitty\ORM\Tools\SchemaTool
     */
    protected $_schemaTool;

    /**
     * @var \Shitty\DBAL\Logging\DebugStack
     */
    ...
    protected function setUp()
    {
        ...
        if ( ! isset(static::$_sharedConn)) {
            ...
            if (static::$_sharedConn->getDriver() instanceof \Shitty\DBAL\Driver\PDOSqlite\Driver) {
        ...
        if ( ! $this->_em) {
            ...
            $this->_schemaTool = new \Shitty\ORM\Tools\SchemaTool($this->_em);
        }
     ...
     *
     * @param \Shitty\ORM\Configuration   $config       The Configuration to pass to the EntityManager.
     * @param \Shitty\Common\EventManager $eventManager The EventManager to pass to the EntityManager.
     *
    ...
    protected function _getEntityManager($config = null, $eventManager = null) {
        ...
        if (is_null(self::$_metadataCacheImpl)) {
            if (isset($GLOBALS['DOCTRINE_CACHE_IMPL'])) {
                ...
                self::$_metadataCacheImpl = new \Shitty\Common\Cache\ArrayCache;
            }
        }

        if (is_null(self::$_queryCacheImpl)) {
            self::$_queryCacheImpl = new \Shitty\Common\Cache\ArrayCache;
        }

        $this->_sqlLoggerStack = new \Shitty\DBAL\Logging\DebugStack();
        $this->_sqlLoggerStack->enabled = false;
        ...
        if (isset($GLOBALS['debug_uow_listener'])) {
            $evm->addEventListener(array('onFlush'), new \Shitty\ORM\Tools\DebugUnitOfWorkListener());
        }

        return \Shitty\ORM\EntityManager::create($conn, $config);
    }
