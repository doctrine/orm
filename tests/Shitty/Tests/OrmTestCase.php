...
use Shitty\Common\Annotations;
use Shitty\Common\Cache\ArrayCache;
use Shitty\Common\Version;
...
abstract class OrmTestCase extends DoctrineTestCase
{
    ...
    /**
     * @var \Shitty\ORM\Cache\Logging\StatisticsCacheLogger
     */
     ...
     * @param mixed $alias
     *
     * @return \Shitty\ORM\Mapping\Driver\AnnotationDriver
     */
    protected function createAnnotationDriver($paths = array(), $alias = null)
    {
        ...
        Annotations\AnnotationRegistry::registerFile(__DIR__ . "/../../../lib/Shitty/ORM/Mapping/Driver/DoctrineAnnotations.php");

        return new \Shitty\ORM\Mapping\Driver\AnnotationDriver($reader, (array) $paths);
    }
     ...
     *
     * @param \Shitty\DBAL\Connection|array    $conn
     * @param mixed                              $conf
    ...
    protected function _getTestEntityManager($conn = null, $conf = null, $eventManager = null, $withSharedMetadata = true)
    {
        ...
        if (is_array($conn)) {
            $conn = \Shitty\DBAL\DriverManager::getConnection($conn, $config, $eventManager);
        }

        return \Shitty\Tests\Mocks\EntityManagerMock::create($conn, $config, $eventManager);
    }
    ...
    /**
     * @return \Shitty\Common\Cache\Cache
     */
