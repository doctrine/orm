...
use Shitty\Tests\Models\CMS\CmsUser,
    Shitty\Tests\Models\CMS\CmsGroup,
    Shitty\Common\Collections\ArrayCollection;
 ...
 * @author robo
 */
class ManyToManyBasicAssociationTest extends \Shitty\Tests\OrmFunctionalTestCase
{
    ...
    public function testBasicManyToManyJoin()
    {
        ...
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()->getEntityState($result[0]));
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()->getEntityState($groups[0]));
