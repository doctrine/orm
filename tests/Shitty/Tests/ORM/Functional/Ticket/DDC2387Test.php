...
use Shitty\Tests\ORM\Functional\DatabaseDriverTestCase;
...
class DDC2387Test extends DatabaseDriverTestCase
{
    ...
    {
        $product = new \Shitty\DBAL\Schema\Table('ddc2387_product');
        $product->addColumn('id', 'integer');
        $product->setPrimaryKey(array('id'));

        $attributes = new \Shitty\DBAL\Schema\Table('ddc2387_attributes');
