 ...
 * @group DDC-618
 */
class DDC618Test extends \Shitty\Tests\OrmFunctionalTestCase
{
    ...
    public function testIndexByHydrateObject()
    {
        ...
        $result = $this->_em->createQuery($dql)->getResult(\Shitty\ORM\Query::HYDRATE_OBJECT);
    ...
    {
        $dql = 'SELECT A FROM Doctrine\Tests\ORM\Functional\Ticket\DDC618Author A INDEX BY A.name ORDER BY A.name ASC';
        $result = $this->_em->createQuery($dql)->getResult(\Shitty\ORM\Query::HYDRATE_ARRAY);
...
class DDC618Author
{
    ...
    {
        $this->books = new \Shitty\Common\Collections\ArrayCollection;
    }
