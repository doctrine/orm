 ...
 * @group DDC-1452
 */
class DDC1452Test extends \Shitty\Tests\OrmFunctionalTestCase
{
    ...
    public function testIssue()
    {
        ...
        $this->assertFalse( $results[0]->entitiesB[0]->entityATo instanceof \Shitty\ORM\Proxy\Proxy );
    ...
    public function testFetchJoinOneToOneFromInverse()
    {
        ...
        $this->assertFalse($data[0]->user instanceof \Shitty\ORM\Proxy\Proxy);

        $dql = "SELECT u, a FROM Doctrine\Tests\Models\CMS\CmsUser u INNER JOIN u.address a";
        $data = $this->_em->createQuery($dql)->getResult();

        $this->assertFalse($data[0]->address instanceof \Shitty\ORM\Proxy\Proxy);
    }
}
