...
use Shitty\ORM\Tools\SchemaTool;
...
class BasicFunctionalTest extends \Shitty\Tests\OrmFunctionalTestCase
{
    ...
    public function testBasicUnitsOfWorkWithOneToManyAssociation()
    {
        ...
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()->getEntityState($user));
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()->getEntityState($ph));
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()->getEntityState($ph2));
    }
    ...
    public function testRemove()
    {
        ...
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_NEW, $this->_em->getUnitOfWork()->getEntityState($user), "State should be UnitOfWork::STATE_NEW");

        $this->_em->persist($user);

        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_MANAGED, $this->_em->getUnitOfWork()->getEntityState($user), "State should be UnitOfWork::STATE_MANAGED");
        ...
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_REMOVED, $this->_em->getUnitOfWork()->getEntityState($user), "State should be UnitOfWork::STATE_REMOVED");
        $this->_em->flush();
    ...
    public function testFlushDoesNotIssueUnnecessaryUpdates()
    {
        ...
        $debugStack = new \Shitty\DBAL\Logging\DebugStack;
    ...
    public function testMergeThrowsExceptionIfEntityWithGeneratedIdentifierDoesNotExist()
    {
        ...
        try {
            $this->_em->merge($user);
            $this->fail();
        } catch (\Shitty\ORM\EntityNotFoundException $enfe) {}
    }
    ...
    public function testManyToOneFetchModeQuery()
    {
        ...
        $article = $this->_em->createQuery($dql)
                             ...
                             ->setFetchMode('Doctrine\Tests\Models\CMS\CmsArticle', 'user', \Shitty\ORM\Mapping\ClassMetadata::FETCH_EAGER)
                             ->getSingleResult();
    ...
    public function testClearWithEntityName()
    {
        ...
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_DETACHED, $unitOfWork->getEntityState($user));
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_DETACHED, $unitOfWork->getEntityState($article1));
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_DETACHED, $unitOfWork->getEntityState($article2));
        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_MANAGED, $unitOfWork->getEntityState($address));

        $this->_em->clear();

        $this->assertEquals(\Shitty\ORM\UnitOfWork::STATE_DETACHED, $unitOfWork->getEntityState($address));
    }
