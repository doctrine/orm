...
namespace Shitty\Tests\ORM\Query;

use Shitty\DBAL\Types\Type as DBALType;
use Shitty\ORM\Query;

class SelectSqlGenerationTest extends \Shitty\Tests\OrmTestCase
{
    private $_em;
    ...
    public function testSupportsAggregateCountFunctionWithSimpleArithmetic()
    {
        ...
        $connMock->setDatabasePlatform(new \Shitty\DBAL\Platforms\MySqlPlatform);
    ...
    public function testSupportsConcatFunctionForMysqlAndPostgresql()
    {
        ...
        $connMock->setDatabasePlatform(new \Shitty\DBAL\Platforms\PostgreSqlPlatform);
        $this->assertSqlGeneration(
            "SELECT u.id FROM Doctrine\Tests\Models\CMS\CmsUser u WHERE CONCAT(u.name, 's') = ?1",
    ...
    public function testSupportsMemberOfExpressionOneToMany()
    {
        ...
        $phone = new \Shitty\Tests\Models\CMS\CmsPhonenumber;
    ...
    public function testSupportsMemberOfExpressionManyToMany()
    {
        ...
        $group = new \Shitty\Tests\Models\CMS\CmsGroup;
        $group->id = 101;
    ...
    public function testSupportsMemberOfExpressionManyToManyParameterArray()
    {
        ...
        $group = new \Shitty\Tests\Models\CMS\CmsGroup;
        $group->id = 101;
        $group2 = new \Shitty\Tests\Models\CMS\CmsGroup;
        $group2->id = 105;
    ...
    public function testSupportsMemberOfExpressionSelfReferencing()
    {
        ...
        $person = new \Shitty\Tests\Models\Company\CompanyPerson;
    ...
    public function testBooleanLiteralInWhereOnSqlite()
    {
        ...
        $this->_em->getConnection()->setDatabasePlatform(new \Shitty\DBAL\Platforms\SqlitePlatform);
    ...
    {
        $oldPlat = $this->_em->getConnection()->getDatabasePlatform();
        $this->_em->getConnection()->setDatabasePlatform(new \Shitty\DBAL\Platforms\PostgreSqlPlatform);
    ...
    {
        if ($this->_em->getConnection()->getDatabasePlatform() instanceof \Shitty\DBAL\Platforms\SqlitePlatform) {
        ...
        $this->assertSqlGeneration(
            ...
            array(Query::HINT_LOCK_MODE => \Shitty\DBAL\LockMode::PESSIMISTIC_WRITE)
        );
    }
    ...
    public function testPessimisticReadLockQueryHintPostgreSql()
    {
        ...
        $this->assertSqlGeneration(
            ...
            array(Query::HINT_LOCK_MODE => \Shitty\DBAL\LockMode::PESSIMISTIC_READ)
                );
    }
    ...
    public function testLockModeNoneQueryHint()
    {
        $this->assertSqlGeneration(
            ...
            array(Query::HINT_LOCK_MODE => \Shitty\DBAL\LockMode::NONE)
                );
    }
    ...
    {
        $this->_em->getConnection()->setDatabasePlatform(new \Shitty\DBAL\Platforms\MySqlPlatform);
    ...
    {
        $this->_em->getConnection()->setDatabasePlatform(new \Shitty\DBAL\Platforms\OraclePlatform);
    ...
    */
    ...
    public function testSupportsMoreThanTwoParametersInConcatFunction()
    {
    	...
    	$connMock->setDatabasePlatform(new \Shitty\DBAL\Platforms\SQLServerPlatform());
    	$this->assertSqlGeneration(
            "SELECT u.id FROM Doctrine\Tests\Models\CMS\CmsUser u WHERE CONCAT(u.name, u.status, 's') = ?1",
            "SELECT c0_.id AS id_0 FROM cms_users c0_ WHERE (c0_.name + c0_.status + 's') = ?"
    	);
...
class MyAbsFunction extends \Shitty\ORM\Query\AST\Functions\FunctionNode
{
     ...
     * @override
     */
    public function getSql(\Shitty\ORM\Query\SqlWalker $sqlWalker)
    {
     ...
     * @override
     */
    public function parse(\Shitty\ORM\Query\Parser $parser)
    {
        $lexer = $parser->getLexer();

        $parser->match(\Shitty\ORM\Query\Lexer::T_IDENTIFIER);
        $parser->match(\Shitty\ORM\Query\Lexer::T_OPEN_PARENTHESIS);

        $this->simpleArithmeticExpression = $parser->SimpleArithmeticExpression();

        $parser->match(\Shitty\ORM\Query\Lexer::T_CLOSE_PARENTHESIS);
    }
}
/**
 * @Entity
