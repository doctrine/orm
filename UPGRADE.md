...
`onDelete`, or `deferrable` value), `SchemaTool` silently generates the
column and foreign key based on whichever of the two associations happens to
be processed first, ignoring the other one entirely.
